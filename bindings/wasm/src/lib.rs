...
where
    T: Into<iota_sdk_evm_bindings_core::Error>,
{
    build_js_error(iota_sdk_evm_bindings_core::Response::Error(err.into()))
}

pub(crate) fn destroyed_err(instance: &str) -> JsError {
    build_js_error(iota_sdk_evm_bindings_core::Response::Panic(format!(
        "{} was destroyed",
        instance
    )))
}
...
where
    T: Into<iota_sdk_evm_bindings_core::Response>,
{
