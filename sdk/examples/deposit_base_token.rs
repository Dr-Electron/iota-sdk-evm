...
use std::{io::Read, str::FromStr, time::{SystemTime, UNIX_EPOCH}};

use instant::Duration;
use iota_sdk::{
    client::Client, packable::PackableExt, types::block::{
        address::Bech32Address,
        output::{
            dto::BasicOutputDto, feature::{MetadataFeature, SenderFeature}, unlock_condition::{AddressUnlockCondition, ExpirationUnlockCondition}, BasicOutputBuilder, Feature, Output
        },
        payload::transaction::TransactionId,
        BlockId,
    }, wallet::{account::types::AccountAddress, Account}, Wallet
};
use iota_sdk_evm::{
...
async fn main() -> Result<()> {
    ...
    if balance.base_coin().available() > 0 {
        ...
        let _ = send_to_evm(&account, to_send, account_addr, None, &api).await?;
...
async fn send_to_evm(
    ...
    let basic_output = BasicOutputBuilder::new_with_amount(amount)
            ...
            .add_unlock_condition(ExpirationUnlockCondition::new(
                from_addr.address(), (now + 3600).try_into().unwrap()
            )?)
            .with_features([
    ...
    //let output: BasicOutputDto = (&basic_output).into();
    let basic = Output::Basic(basic_output);
    let bytes = prefix_hex::encode(basic.pack_to_vec());
    let value = serde_json::to_value(bytes).unwrap();
    println!("value: {:?}", value);
    
    let res = api.estimate_gas_on_ledger(TESTNET_CHAIN_ADDRESS, value).await?;
    println!("estimate_gas_on_ledger: {:?}", res);
